...
signed main() {
    ...
    for (int i = 1, r; i <= n; i++) {
        std::cin >> r;
        mcmf.add_edge(S, 2 * i, r, 0);
        mcmf.add_edge(2 * i - 1, T, r, 0);
        if (i + 1 <= n) {
            mcmf.add_edge(2 * i, 2 * i + 2, inf, 0);
        }
    }
    int p, m, f, _n, s;
    std::cin >> p >> m >> f >> _n >> s;
    for (int i = 1; i <= n; i++) {
        mcmf.add_edge(S, 2 * i - 1, inf, p);
        if (i + m <= n) {
            mcmf.add_edge(2 * i, 2 * (i + m) - 1, inf, f);
        }
        if (i + _n <= n) {
            mcmf.add_edge(2 * i, 2 * (i + _n) - 1, inf, s);
        }
    }
