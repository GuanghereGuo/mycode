...
int main() {
    ...
    int n;
    std::cin >> n;
    MinCostMaxFlow min_cmf(n * 2 + 2);
    MinCostMaxFlow max_cmf(n * 2 + 2);
    const int s = n * 2 + 1, t = n * 2 + 2;

    for (int i = 1; i <= n; i++) {
        min_cmf.add_edge(s, i, 1, 0);
        max_cmf.add_edge(s, i, 1, 0);
        min_cmf.add_edge(n + i, t, 1, 0);
        max_cmf.add_edge(n + i, t, 1, 0);
        // min_cmf.add_edge(i, t, 1, 0);
        // max_cmf.add_edge(i, t, 1, 0);
    }

    for (int i = 1, x; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            std::cin >> x;
            min_cmf.add_edge(i, n + j, 1, x);
            max_cmf.add_edge(i, n + j, 1, -x);
        }
    }
