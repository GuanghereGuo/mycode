...
实验三的 Multisim 参考仿真电路如图~\ref{e3-fig:thevenin-multisim} 所示。实验时先测量原线性有源网络的开路电压、短路电流和负载端口伏安特性，再根据测得的 \(U_{oc}\) 与 \(R_{\mathrm{th}}\) 搭建戴维南等效电路，最后比较两种电路在相同负载下的端口电压与电流。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{image6.png}
    \caption{实验三 Multisim 参考仿真电路}
    \label{e3-fig:thevenin-multisim}
\end{figure}
...
\begin{table}[H]
    ...
    \label{e3-tab:simulation-result}
    \begin{tabular}{ccccc}
        \toprule
...
\begin{figure}[H]
    ...
    \caption{实验三实物电路连接}
    \label{e3-fig:physical-circuit}
\end{figure}

\section{实验数据}

实验原始数据记录如图~\ref{e3-fig:raw-data} 所示。整理时仅录入原始记录中未被划掉且能够确定辨认的有效数据；被划掉的数据和未记录数据不作为有效实验数据使用。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\textwidth]{raw_data_upright.jpg}
    \caption{实验三原始数据记录照片}
    \label{e3-fig:raw-data}
\end{figure}

\begin{table}[H]
    \centering
    \caption{等效内阻测量数据整理}
    \label{e3-tab:rth-measurement}
    \begin{tabular}{cccc}
        \toprule
...
\begin{equation}
    R_{\mathrm{th}}=\frac{U_{oc}}{I_{sc}\times 10^{-3}},
\end{equation}
其中 \(I_{sc}\) 的单位为 \(\mathrm{mA}\)。由于原始有效记录中未明确保留短路电流读数，表~\ref{e3-tab:rth-measurement} 中主要采用直接测量法得到的等效内阻。该结果与预习仿真得到的 \(177.71\,\Omega\) 接近，相对偏差为
\begin{equation}
