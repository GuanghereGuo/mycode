% ==========================================
% 实验任务：方波分解 (占空比 40%, 含8f以上)
% ==========================================
clc; clear; close all;

% --- 1. 参数设置 ---
f_base = 400;       % <--- 修改这里：400 或 600
E = 2;              % 信号幅度 2V
duty = 0.4;         % 占空比 40%
...
for i = 1:length(harmonics) - 1
    n = harmonics(i);
    theory_values(i) = 2 * abs( (2 * E) / (n * pi) * sin(n * pi * duty) );
end
...
% --- 3. 实验测量值 (请填入8个数据) ---
% !!! 请替换真实数据 !!!
% 40%占空比：5f应极小，8f应该有数值
measured_values = [2.56, 1.04, 0.80, 0.88, 0, 0.72, 0.52, 0.32]; 

% --- 4. 绘图 ---
figure('Name', ['40%占空比频谱 (f=' num2str(f_base) ')'], 'Color', 'w');
...
title(['理论值频谱 (Duty=40%, f=' num2str(f_base) 'Hz)']);
ylabel('峰峰值 (V)');
grid on;
xticks(freqs);
...
title(['实验测量值频谱 (Duty=40%, f=' num2str(f_base) 'Hz)']);
xlabel('频率 (Hz)');
ylabel('峰峰值 (V)');
grid on;
xticks(freqs);
