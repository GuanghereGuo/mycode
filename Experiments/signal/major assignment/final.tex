...
通过计算归一化互相关函数来实现：
\begin{enumerate}
    \item \textbf{参考模板}：取已合成信号尾部波形。
    \item \textbf{滑动搜索}：在原信号 $L_k$ 附近的窗口 $[L_k - \Delta, L_k + \Delta]$ 内滑动对比。
    \item \textbf{寻找峰值}：互相关峰值位置即为最佳匹配点 $\delta^*$。
\end{enumerate}

如图 \ref{fig:wsola_search_process} 所示，WSOLA 通过引入偏移量 $\delta^*$ 实现了波形相位对齐。
