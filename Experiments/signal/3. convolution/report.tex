...
\subsubsection{加法器}
直流输入下的测试数据如表\ref{tab:adder-data}所示。
\begin{table}[htbp]
    \centering
    \caption{加法器直流输入测试数据}
    ...
    \end{tabular}
\end{table}
由上表可知：输出电压大致等于两路输入电压之和取反相，验证了加法器的功能。

当输入一改为峰峰值为 \SI{2}{\volt}、频率为 \SI{500}{\hertz} 的方波时，波形如图\ref{fig:result-adder}所示。从图中可以看出，加法器实现了对输入方波的直流偏置与反相。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/反相加法器.jpg}
    \caption{加法器混合输入波形（方波与直流）}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ccc}
        \toprule
        电路参数 & 输入 $V_{pp}$ (\si{\volt}) & 输出 $V_{pp}$ (\si{\milli\volt}) \\
        \midrule
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/积分器.jpg}
    \caption{积分器输入输出波形（输入为方波）}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/系统模拟.jpg}
