...
根据测试方法，测量串联电阻 $R_s$ 两端的电压 $V_{Rs}$ 和放大电路输入端的电压 $V_i$。
\begin{itemize}
    \item 串联电阻两端电压 $V_{Rs} = \SI{25.716}{\milli\volt}$
    \item 电路输入端电压 $V_i = \SI{390.65}{\milli\volt}$
\end{itemize}
输入电阻 $R_i$ 计算如下：
\begin{equation}
    R_i = \frac{V_i}{I_i} = \frac{V_i}{V_{Rs} / R_s} = \frac{V_i}{V_{Rs}} \times R_s = \frac{\SI{390.65}{\milli\volt}}{\SI{25.716}{\milli\volt}} \times \SI{1}{\kilo\ohm} \approx \SI{15.19}{\kilo\ohm}
\end{equation}
实验测得输入电阻约为 \SI{15.19}{\kilo\ohm}，与理论值 \SI{2.70}{\kilo\ohm} 存在较大差异。这主要是因为理论计算中采用的晶体管 $\beta$ 值 (150) 可能远小于实际值。

\paragraph{2. 输出电阻 $R_o$ 测量：}
测量空载（断开 $R_{L1}$）时的输出电压 $V_o$ 和接入负载 $R_{L1} = \SI{1}{\kilo\ohm}$ 时的输出电压 $V_L$。测量读数如图 \ref{fig:res_out_measure} 所示。
\begin{itemize}
    \item 空载输出电压 $V_o = \SI{155.693}{\milli\volt}$
    \item 带载输出电压 $V_L = \SI{116.472}{\milli\volt}$
\end{itemize}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/1_Co_shot/vo.jpg}
...
\begin{equation}
    R_o = \left( \frac{V_o}{V_L} - 1 \right) \times R_{L1} = \left( \frac{\SI{155.693}{\milli\volt}}{\SI{116.472}{\milli\volt}} - 1 \right) \times \SI{1}{\kilo\ohm} \approx \SI{337}{\ohm}
\end{equation}
实验测得输出电阻约为 \SI{337}{\ohm}，远小于理论估算值 \SI{2}{\kilo\ohm}。这是因为理论计算 $R_o \approx R_{c1}$ 是一个粗略近似，实际的输出电阻是 $R_{c1}$ 与晶体管自身输出电阻 $r_{ce}$ 的并联值。
