...
\begin{table}[htbp]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        可变电阻 $R_p$ (\unit{\kilo\ohm}) & 4.749 & 8.792 & 9.203 \\
        反馈电压 $V_{fp}$ (\unit{\volt}) & 0.616 & 6.639 & 7.546 \\
        输出电压 $V_{op}$ (\unit{\volt}) & 1.777 & 19.968 & 22.749 \\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{3. 不同电容值下的输出波形 ($R_p = \qty{8.643}{\kilo\ohm}$)}
...
\begin{table}[H]
    \centering
    \caption{不同电容值下的理论与实测数据对比 ($R_p = \qty{8.643}{\kilo\ohm}$)}
    \label{tab:cap_data_4}
    \resizebox{\textwidth}{!}{% 自动缩放表格以适应页面宽度
    \begin{tabular}{lcccccc}
        \toprule
        \multirow{2}{*}{电容条件} & \multicolumn{3}{c}{理论值} & \multicolumn{3}{c}{实测值} \\
        \cmidrule(lr){2-4} \cmidrule(lr){5-7}
         & $V_{op}$ (\unit{\volt}) & $f$ (\unit{\Hz}) & $V_f$ (\unit{\volt}) & $V_{op}$ (\unit{\volt}) & $f$ (\unit{\Hz}) & $V_f$ (\unit{\volt}) \\
        \midrule
        $C=\qty{0.01}{\micro\farad}$ & 17.432 & 1.5915k & 5.634 & 17.237 & 1.5413k & 5.773 \\
        $C=\qty{0.02}{\micro\farad}$ & 18.125 & 795.77 & 6.183 & 18.218 & 776.63 & 6.0842 \\
        \bottomrule
    \end{tabular}
    }
\end{table}
...
\begin{enumerate}
    \item \textbf{数据分析}：由表 \ref{tab:three_states_4} 可知，当 $R_p = \qty{8.792}{\kilo\ohm}$ 时，输出波形振幅最大且不失真，此时 $V_{fp} = \qty{6.639}{\volt}$，$V_{op} = \qty{19.968}{\volt}$。由表 \ref{tab:cap_data_4} 可知，在 $R_p = \qty{8.643}{\kilo\ohm}$ 时，输出电压、频率及反馈电压的实测值与理论值误差较小。
