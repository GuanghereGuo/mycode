...
- 撰写电基报告的实验原理、实验过程/步骤时，优先识别当前实验未命名 PPT 实拍图中的文字内容，再用 `CircuitTheory/reference.pdf` 补充核对
...
- 数电实验要求必须严格来自当前实验目录内的说明文件/说明图片；不得用 `参考资料/` PDF 或 `别人的报告/` 替代、补写或推断实验要求
- 若数电实验要求在图片中，必须先 OCR/人工识别并校对为文字写入“实验要求”；不得直接插入要求图片代替文字
- 若实验目录无单独说明文件，撰写数电报告前必须先查看 `DigitalElectronics/参考资料/` 中对应实验要求 PDF
- `DigitalElectronics/别人的报告/` 中有已完成报告，仅可参考格式和表达风格，不可替代当前实验目录内的要求
- 用户提前放入实验 `figures/` 的图片必须按内容和文件名判断用途，并插入报告合适位置，不得遗漏
- 数电报告“实验内容”以操作步骤为主：建工程、放元件、连线、编译、仿真、引脚分配、下载验证；少写泛泛原理
- 数电实验目录缺少 `.v`/`.bdf`/Quartus 工程时，不编造源码清单或工程细节；只基于当前说明、截图和可见材料撰写
...
- **报告正文禁止使用括号补充说明**：不要用圆括号对术语或条件进行解释，应将信息融入正文句子。例如写"了解 RC 低通和高通无源滤波器的基本工作原理"而非"了解 RC 无源滤波器（低通、高通）的基本工作原理"
- **除非必要，避免使用破折号和顿号**：破折号插入语改为独立句子或用逗号衔接；顿号列举改为"和""与""及"等连词
