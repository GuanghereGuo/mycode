...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_runs) as executor:
        futures = [executor.submit(sa) for _ in range(num_runs)]
        results = [future.result() for future in concurrent.futures.as_completed(futures)]

    best_result = min(results, key=lambda x: x[1])
    best_assign, best_obj, obj_history = best_result
    ...
    print(f"Final Total Dissatisfaction: {final_diss:.4f}")
    print(f"Final Total Negative Impact: {final_neg_impact:.4f}")
