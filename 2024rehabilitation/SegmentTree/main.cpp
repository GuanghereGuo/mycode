...
class SegmentTree {
    ...
    void push_down(int now, int l, int r) {
        ...
        int mid = l + (r - l) / 2;
        (tree[now << 1].val += tree[now].lzy * (mid - l + 1));
        (tree[now << 1 | 1].val += tree[now].lzy * (r - mid));
        (tree[now << 1].lzy += tree[now].lzy);
        (tree[now << 1 | 1].lzy += tree[now].lzy);
        tree[now].lzy = 0;
    }
    void add(int now, int l, int r, int tar_l, int tar_r, T val) {
        if (tar_l <= l && r <= tar_r) {
            (tree[now].val += val * (r - l + 1));
            (tree[now].lzy += val);
            return;
        }
        push_down(now, l, r);
    ...
    T query(int now, int l, int r, int tar_l, int tar_r) {
        ...
        T res = 0;
        if (tar_l <= mid) {
            (res += query(now << 1, l, mid, tar_l, tar_r));
        }
        if (tar_r > mid) {
            (res += query(now << 1 | 1, mid + 1, r, tar_l, tar_r));
        }
        return res;
    }
...
signed main() {
    ...
    SegmentTree<long long> seg(arr, 1e8 + 1);
